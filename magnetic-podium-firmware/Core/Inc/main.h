...
#include "config.h" // Добавлено: теперь HID_JoystickReport_TypeDef, Position3D_t, SensorGeometry_t видны
#include "debug_console.h" // Для LOG_LEVEL_*
...
#define Coil3_DIR_Pin GPIO_PIN_2
#define Coil3_DIR_GPIO_Port GPIOE
...
// Определены в levitation_control.c
extern SystemState_t system_state;
extern OperationMode_t current_mode;

// --- ГЛОБАЛЬНАЯ ПЕРЕМЕННАЯ HID ОТЧЕТА ---
extern HID_JoystickReport_TypeDef Joystick_Report;

// --- ВНЕШНИЕ ПЕРЕМЕННЫЕ ДАТЧИКОВ ---
// УБРАНО: extern MLX90393_t sensors[NUM_SENSORS]; // <-- УБРАНО ОТСЮДА
