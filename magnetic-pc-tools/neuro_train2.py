...
def main():
    ...
    print("Поместите шар в центр каркаса и держите его рукой.")
    ...
    # Стартовые коэффициенты (можно аккуратно повышать)
    pid_x = PID(0.3, 0.0, 0.05) # Увеличили жесткость X
    pid_y = PID(0.3, 0.0, 0.05) # Увеличили жесткость Y
    pid_z = PID(0.5, 0.01, 0.05)
    ...
    try:
        while True:
            ...
            ux, uy, uz = pid_x.compute(dx), pid_y.compute(dy), pid_z.compute(dz)
            
            powers = compute_coil_powers(ux, uy, uz, current_limit)
            ser.write(MARKER + struct.pack('<12f', *powers))

            if time.time() % 1 < 0.05:
                # Теперь мы печатаем РЕАЛЬНЫЙ ток на катушке (powers[4]), а не абстрактный уз
                print(f"Err_Z: {dz:7.1f} | Coil_Z: {powers[4]:5.2f} | Limit: {current_limit:.2f}")
